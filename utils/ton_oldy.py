...
class TonOldy:
    ...
    def generate_word(self, word_hash:str, word_length: int):
        ...
        for combo in itertools.product(string.ascii_lowercase, repeat=word_length):
            ...
            if base64.b64encode(hashlib.sha256((word + '_stop_hacking_us').encode('utf-8')).digest()).decode('utf-8') == word_hash:
                logger.info(f"Thread {self.thread} | {self.account} | Picked the word «{word}»")
                return word
        else:
            logger.warning(f"Thread {self.thread} | {self.account} | Couldn't pick the word :(")
            None
