...
class TonOldy:
    ...
    def generate_word(self, word_hash:str, word_length: int):
        tot_var = len(string.ascii_lowercase) ** word_length
        logger.info(f"Thread {self.thread} | {self.account} | Picking a word... Total variations: {tot_var}")

        for combo in itertools.product(string.ascii_lowercase, repeat=word_length):
            word = ''.join(combo)

            if base64.b64encode(hashlib.sha256(word.lower().encode('utf-8')).digest()).decode('utf-8') == word_hash:
                logger.info(f"Thread {self.thread} | {self.account} | Picked the word «{word}»")
                return word
        else:
            logger.warning(logger.info(f"Thread {self.thread} | {self.account} | Couldn't pick the word :("))
            None

    async def submit_daily_hunts(self, word: str):
        resp_txt = await (await self.session.post(f'https://backend.tonoldy.com/api/challenge/daily_hunt?dailyHuntWord={word}')).text()
        return resp_txt == ''
